...
ai_buffer g_audio_data_map_activations[AI_AUDIO_DATA_ACTIVATIONS_COUNT] = {
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 2500, 1, 1),
    2500, NULL, NULL),    /* heap_overlay_pool */
  };
ai_buffer g_audio_data_map_weights[AI_AUDIO_DATA_WEIGHTS_COUNT] = {
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 46120, 1, 1),
    46120, NULL, s_audio_weights_array_u64),   /* weights_array */
  };
...
ai_buffer ai_audio_data_activations_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_AUDIO_DATA_ACTIVATIONS_SIZE, 1, AI_AUDIO_DATA_ACTIVATIONS_COUNT),
    AI_AUDIO_DATA_ACTIVATIONS_SIZE,
    NULL, ptr);
  return buf;
}
...
ai_buffer ai_audio_data_weights_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_AUDIO_DATA_WEIGHTS_SIZE, 1, AI_AUDIO_DATA_WEIGHTS_COUNT),
    AI_AUDIO_DATA_WEIGHTS_SIZE,
    NULL, ptr);
  return buf;
}
...
{
  return AI_HANDLE_PTR(g_audio_weights_table);
